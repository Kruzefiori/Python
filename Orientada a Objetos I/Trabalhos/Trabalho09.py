...
if __name__ == '__main__':
    ...
    for name, end , age , cpf , curso in ListaExemplo:
        try:
            ...
            print('Usuário Não cadastrado\nIdade inferior a 18 anos! idade {} do usuario:{}\n'.format(age , name))
        except cpf_duplicado:
            print('Usuário Não cadastrado\nCpf {} já cadastrado no sistema, do usuario:{}\n'.format(cpf, name))
        except curso_nao_aceito:
            print('Usuário Não cadastrado\nCurso não aceito no cadastro {} do usuario:{}\n'.format(curso, name))
        else:
            newAluno = Aluno(name, end, age, cpf, curso)
            cadastrados[cpf] = newAluno
    ...
    for name, end , age , cpf , titulo in ListaExemploDois:
        try:
            ...
            cadastrados[cpf] = newprof
            print('Usuário %s cadastrado com sucesso\n' %name)
    print('Usuário Cadastrados:')
    for each in cadastrados:
        cadastrados[each].printDescricao()